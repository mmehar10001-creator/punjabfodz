  ...
  head: () => ({
    meta: [
      { title: "Catering & Deg Menu — Punjab Foods" },
      {
        name: "description",
        content:
          "Order Pulao, Biryani, Qorma and Zarda Degs for weddings and events. Sada Pulao Deg from Rs. 10,000.",
      },
      { property: "og:title", content: "Catering & Deg — Punjab Foods" },
      {
        property: "og:description",
        content: "Full Deg menu for shadi, gham and events. One call away.",
      },
    ],
  }),
        ...
        <div className="mx-auto max-w-5xl">
          <div className="overflow-hidden border border-border">
            <table className="w-full">
              <thead className="bg-card">
                <tr>
                  <th className="text-left p-5 text-xs uppercase tracking-[0.2em] text-primary">
                    Dish
                  </th>
                  <th className="text-left p-5 text-xs uppercase tracking-[0.2em] text-primary hidden md:table-cell">
                    Quantity
                  </th>
                  <th className="text-right p-5 text-xs uppercase tracking-[0.2em] text-primary">
                    Price
                  </th>
                </tr>
              </thead>
              <tbody>
                {degMenu.map((d, i) => (
                  <tr key={i} className="border-t border-border hover:bg-card/50 transition-colors">
                    ...
                    <td className="p-5 text-muted-foreground text-sm hidden md:table-cell">
                      {d.qty}
                    </td>
                    <td className="p-5 text-right text-primary font-medium whitespace-nowrap">
                      {d.price}
                    </td>
                  </tr>
                ))}
          ...
          <div className="mt-16 text-center border border-primary/30 p-10 bg-card">
            ...
            <p className="text-muted-foreground mb-6">
              Call us directly for catering bookings and bulk orders.
            </p>
            <div className="flex flex-wrap gap-4 justify-center">
              {contact.phones.map((p) => (
                <a
                  ...
                  className="text-sm tracking-wider border border-primary px-6 py-3 text-primary hover:bg-primary hover:text-primary-foreground transition-all"
                >
                  {p}
                </a>
              ))}
